...
module.exports = (robot) => {
	...
	robot.on('ibmcloud.activity', (res, parameters) => {
		...
		if (parameters && parameters.timeframe) {
			...
			return getUsageReport(robot, res, timeframe).then((result) => {
				robot.logger.info(`${TAG}: Successful async call to getUsageReport() returned with result:${JSON.stringify(result)}`);
				if (Array.isArray(result)) {
					robot.emit('ibmcloud.formatter', { response: res, attachments: result});
				}
				else {
			...
			robot.logger.error(`${TAG}: Unable to parse timeframe from the request.`);
			res.reply(i18n.__('error.process.timeframe'));
		}
	});

	// setup response for messages coming in via the hubot adapter.
	robot.respond(COMMAND_USAGE_REPORT, {id: 'ibmcloud.activity'}, (res) => {
		robot.logger.debug(`${TAG}: COMMAND_USAGE_REPORT res.message.text=${res.message.text}.`);
		robot.logger.info(`${TAG}: Getting activity report...`);
